...
- [x] B2 — Un solo reparto de huecos por semana para todos los mercados, con ranking global. Se respetan `max_posiciones`, `max_por_sector`, `max_por_mercado` y el efectivo; el percentil de momentum y la puntuación fundamental usan la cohorte de todos los mercados (el recurso al bloque ya puede actuar). Comprobado: el motor antiguo abría 5 y 6 posiciones con un tope de 4. Registro v0.4.1 en `ESTRATEGIA.md` y supuesto en `SUPUESTOS.md`. Tests en `tests/test_reparto_global.py` (misma rama, pendiente de fusionar).
- [x] B3 — El deslizamiento ya no se resta dos veces: el resultado de cada operación es exactamente lo que se movió en caja. Comprobado con datos sintéticos: antes la suma de resultados daba 480 € menos que la ganancia real de la caja; ahora cuadra al céntimo. La curva de capital no cambia. Registro v0.4.2 en `ESTRATEGIA.md`. Tests en `tests/test_deslizamiento.py` (misma rama, pendiente de fusionar).
- [x] B4 — El calentamiento se cuenta desde el principio de los datos: con histórico previo se decide desde la primera semana (con datos sintéticos, un backtest que empieza en 2023 hacía su primera compra en febrero de 2024; ahora, la primera semana). La rentabilidad por año encadena bien y la curva termina tras liquidar. Registro v0.4.3 en `ESTRATEGIA.md`. Tests en `tests/test_calentamiento.py` (misma rama, pendiente de fusionar).
- [x] B5 — Percentiles sin contar los huecos; EV con el precio sin ajustar (también en el diagnóstico); sin deuda neta no hay EV; cada orden anota su cohorte y el informe avisa de las compras puntuadas contra el bloque o con cohorte insuficiente (el recurso al bloque actúa desde B2). Registro v0.4.4 en `ESTRATEGIA.md`. Tests en `tests/test_puntuacion_fundamental.py` (misma rama, pendiente de fusionar).
- [x] B6 — Descargas robustas: filas malas reparadas o apartadas con aviso; cada tipo de dato se guarda por separado (si fallan los fundamentales se conservan los anteriores y el comando termina con código 1); reintentos en estados financieros y pausas entre peticiones (`implementacion.descargas`); error si falta una divisa o su cambio tiene más de `datos.fx_antiguedad_maxima_dias` (7); volumen con huecos ya no pasa el filtro de liquidez; la sesión en curso se aparta; el diagnóstico mira el ejercicio más reciente y comprueba índices, divisas y ETF. Registro v0.4.5 en `ESTRATEGIA.md`. Tests en `tests/test_descargas.py` (misma rama, pendiente de fusionar). Se comprobará con datos reales en C1.
- [x] B7 — La caja ya no se resta dos veces en la deuda neta de yfinance (`deuda_neta_de`, ejercicio a ejercicio). Tests para `ajustar_ohlc`, `con_reintentos` y `estimar_fecha_publicacion`; uno de ellos destapó que un cierre de 0 rompía `ajustar_ohlc`, también corregido. Registro v0.4.6 en `ESTRATEGIA.md`. Tests en `tests/test_yfinance.py` (misma rama, pendiente de fusionar).
- [x] B8 — Un sector que el proveedor devuelve y el mapeo no conoce rechaza el valor; el `sector_declarado` solo se usa si el proveedor no da sector. El diagnóstico imprime el YAML listo para pegar (con la categoría declarada si todos los valores coinciden; comentado si no). Registro v0.4.7 en `ESTRATEGIA.md`. Tests en `tests/test_sectores.py` (misma rama, pendiente de fusionar).
- [x] B9 — Fecha de corte fija en `validacion.fecha_corte` (propuesta: 2024-05-01, el 70 % de 8 años de datos). `informe` muestra por defecto solo el diseño; `--periodo todo` anota la consulta igual que `validacion`; el panel pide confirmación, anota una vez por sesión y la vista de señales ya no enseña el historial. Registro v0.4.8 en `ESTRATEGIA.md`. Tests en `tests/test_validacion.py` (misma rama, pendiente de fusionar).
- [x] B10 — Limpieza: `foto` ya funciona en una carpeta recién clonada; una venta sin precio se aplaza (`venta_aplazada`) en lugar de perderse; los `except Exception` del tipo de cambio solo atrapan errores de datos y anotan el motivo (`sin_tipo_de_cambio`); `senales --detalle` enseña los rechazos de la última revisión; los errores de configuración salen como un mensaje claro con el fichero y la clave; seis números sueltos pasan a `config/` con los mismos valores; fuera el código muerto de `tecnico.py`; tildes y eñes en el informe y el panel. Registro v0.4.9 en `ESTRATEGIA.md`. Tests en `tests/test_limpieza.py` (misma rama, pendiente de fusionar). **Fase B terminada.**
