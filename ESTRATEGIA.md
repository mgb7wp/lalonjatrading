...
| 2026-09-23 | 0.4.1   | Corrección: el reparto semanal de huecos es uno solo para todos los mercados. Las candidatas de los cinco mercados se ordenan en una lista global y los topes (`max_posiciones`, `max_por_sector`, `max_por_mercado`) y el efectivo se consumen una sola vez; la puntuación fundamental y el percentil de momentum se calculan con la cohorte de todos los mercados, de modo que el recurso al bloque funciona; las posiciones ya en cartera cuentan en la cohorte del momentum | El motor repartía mercado a mercado: podía superar los topes y el efectivo, y entraba antes el mercado que iba primero en la lista, no la mejor candidata. No es un cambio de regla: alinea el código con este documento | — |
| 2026-09-23 | 0.4.2   | Corrección: el deslizamiento se cuenta una sola vez por operación. Los precios de compra y venta ya lo incluyen, así que el resultado de cada operación resta solo comisión e impuesto; el coste total anotado sigue incluyendo el deslizamiento | El resultado por operación salía peor de lo real: afectaba al % de ganadoras y a los resultados por mercado y por bloque, sobre todo en emergentes. La curva de capital no cambia | — |
| 2026-09-23 | 0.4.3   | Corrección: el calentamiento de los indicadores se cuenta desde el principio de los datos, así que un backtest que empieza con histórico previo decide desde la primera semana; la rentabilidad de cada año se mide desde el cierre del año anterior; la curva termina después de liquidar las posiciones abiertas | El periodo de validación pasaba más de un año en liquidez esperando indicadores que ya tenía; los años encadenados no daban la rentabilidad total; el capital final no incluía los costes de la última venta | — |
| 2026-09-23 | 0.4.4   | Corrección de la puntuación fundamental: los huecos no cuentan en el tamaño de la muestra de los percentiles; el EV se calcula con el precio sin ajustar por dividendos (`cierre_bruto`); sin deuda neta no hay EV (antes se tomaba como cero); el informe cuenta las compras puntuadas contra el bloque o con cohorte insuficiente | Los percentiles salían más bajos cuanto más huecos había; el EV pasado salía menor que el real; una empresa sin dato de deuda parecía barata; el recurso al bloque no se veía en el informe | — |
| 2026-09-23 | 0.4.5   | Descargas robustas: las filas malas de precios se reparan o se apartan con aviso; la sesión en curso no se usa; un volumen que falta cuenta como sesión sin negociación en el filtro de liquidez; nuevo `datos.fx_antiguedad_maxima_dias` (7) y error si falta una divisa o su cambio está congelado; cada tipo de dato se guarda por separado; reintentos y pausas configurables en `implementacion.descargas`; el diagnóstico mira el ejercicio más reciente y comprueba índices, divisas y ETF de referencia | Con datos reales, una fila rara tumbaba la descarga entera, un fallo de fundamentales perdía los precios, un volumen con huecos pasaba el filtro de liquidez y una divisa que faltaba pasaba en silencio | — |
| 2026-09-23 | 0.4.6   | Corrección del adaptador de yfinance: cuando Yahoo solo da la deuda neta, ya no se le vuelve a restar la caja; un cierre de 0 ya no rompe el ajuste de precios | La deuda neta salía menor que la real en las empresas con caja: pasaban el filtro de deuda/EBITDA con más facilidad y parecían más baratas | — |
| 2026-09-23 | 0.4.7   | Corrección: un sector que el proveedor devuelve y el mapeo no conoce rechaza el valor, aunque tenga `sector_declarado`; el respaldo de universo.yaml solo se usa si el proveedor no da sector. El diagnóstico imprime el YAML que hay que pegar en `sectores` | Contradecía la regla de `SUPUESTOS.md`: una etiqueta nueva del proveedor, que podría ser la de un banco, se tapaba con la clasificación escrita a mano | — |
| 2026-09-23 | 0.4.8   | Nuevo `validacion.fecha_corte` (2024-05-01): el corte entre diseño y validación queda fijo en lugar de salir de `fraccion_diseno` sobre los datos descargados. `informe` muestra por defecto solo el diseño; `--periodo todo` también anota la consulta; el panel pide confirmación y anota una vez por sesión | El corte avanzaba cada semana con los datos nuevos, y el informe semanal y el panel enseñaban la validación sin anotarla | — |
| 2026-09-23 | 0.4.9   | Limpieza: una venta programada sin precio se aplaza a la siguiente sesión en lugar de perderse; pasan a la configuración, con los mismos valores que tenían en el código, `cartera.margen_reserva_pct` (0,01), `datos.dias_maximos_sin_precio` (10), `tecnico.holgura_historial_factor` (1,5) y `holgura_historial_dias` (30), `metricas.fraccion_periodo_muerto_aviso` (0,10) y `validacion.consultas_para_alarma` (3) | Números sueltos en el código, que el documento pide que vivan en la configuración; una venta decidida no puede desaparecer sin dejar rastro | — |
| 2026-09-23 | 0.5.0   | Integración de las correcciones 0.4.1–0.4.9 en la plataforma La Lonja. `validacion.fecha_corte` pasa de 2024-05-01 a 2020-09-01: el histórico de referencia son los 20 años de la base de datos de la plataforma (septiembre 2006 – septiembre 2026) y no los 8 de una descarga por línea de comandos; el 70 % (`fraccion_diseno`) cae en septiembre de 2020. Un sector que ya es una categoría de este documento (`tecnologia`, `electricas`...) se admite sin traducir. La descarga programada tras el cierre de cada mercado conserva la sesión del día. Los perfiles de puntuación pasan a la versión 1.1.0 | La fecha de 0.4.8 estaba pensada para 8 años de datos y dejaba el diseño en 18 de los 20 años de la base. Sin la regla de categorías, el backtest desde la base de datos rechazaba todos los valores; sin la de la sesión, la plataforma iba siempre un día por detrás. Los scores cambian con 0.4.4 y no deben mezclarse con los anteriores | — |
