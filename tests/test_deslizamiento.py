...
def _posicion(**kwargs):
    base = dict(
        ...
        divisa="EUR",
        acciones=10,
        fecha_entrada=dt.date(2023, 1, 2),
        precio_entrada_local=100.0,
        precio_entrada_base=100.0,
        fx_entrada=1.0,
        atr_entrada=2.0,
        stop_inicial_local=96.0,
        maximo_cierre_local=100.0,
        stop_dinamico_local=94.0,
        coste_entrada_base=0.0,
        riesgo_teorico_pct=0.01,
        riesgo_efectivo_pct=0.008,
...
def test_el_resultado_no_resta_otra_vez_el_deslizamiento():
    ...
    pos = _posicion(
        acciones=acciones,
        precio_entrada_local=101.0,
        precio_entrada_base=101.0,
    ...
    op = cartera.cerrar(
        ...
        ingreso,
        3.0 + desliz_salida,
        MotivoSalida.STOP_INTRADIA,
        deslizamiento_salida_base=desliz_salida,
    )
...
def test_una_operacion_casi_plana_no_se_vuelve_perdedora():
    ...
    pos = _posicion(
        acciones=1,
        precio_entrada_local=100.3,
        precio_entrada_base=100.3,
        coste_entrada_base=d_in,
        deslizamiento_entrada_base=d_in,
    )
    ...
    op = cartera.cerrar(
        ...
        d_out,
        MotivoSalida.STOP_INTRADIA,
        deslizamiento_salida_base=d_out,
    )
...
def _primer_ticker(cfg, mercado):
    return next(t for t in cfg.universo.tickers() if cfg.universo.mercado_de_ticker[t] == mercado)
...
def test_el_resultado_cuadra_con_la_caja_en_cada_mercado(cfg, instantanea, mercado):
    ...
    orden = Orden(
        ...
        mercado=mercado,
        sector="industrial",
        fecha_decision=entrada - dt.timedelta(days=3),
        acciones=100,
        stop_inicial_local=0.01,
        atr_entrada=1.0,
        rango_asignacion=1,
        puntuacion_final=50.0,
        riesgo_teorico_pct=0.01,
        riesgo_efectivo_pct=0.01,
        limitada_por_peso_maximo=False,
    )
    eventos: list = []
    backtest_mod._abrir(cartera, orden, entrada, instantanea.vista(entrada), cfg, eventos, divisa)
