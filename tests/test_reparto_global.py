...
def _senal(ticker, mercado, momentum, fecha):
    ...
    return SenalTecnica(
        ...
        fecha=fecha,
        cierre=precio,
        media_corta=precio * 0.98,
        media_larga=precio * 0.95,
        momentum=momentum,
        atr=precio * 0.06,
...
def _posicion(ticker, mercado, sector, valor=10.0):
    ...
    return Posicion(
        ...
        divisa="EUR",
        acciones=1,
        fecha_entrada=dt.date(2024, 1, 2),
        precio_entrada_local=valor,
        precio_entrada_base=valor,
        fx_entrada=1.0,
        atr_entrada=1.0,
        stop_inicial_local=valor * 0.7,
        maximo_cierre_local=valor,
        stop_dinamico_local=valor * 0.7,
...
def test_las_ordenes_van_a_la_sesion_de_ejecucion_de_su_mercado(cfg_tecnico, semana):
    ...
    ronda = _ronda_llena(cfg, semana)
    pendientes: dict = {}
    _repartir(max(decisiones.values()), ronda, _VistaVacia(), cal, cartera, cfg, pendientes, [])
...
def test_el_percentil_recurre_al_bloque_con_la_cohorte_de_todos_los_mercados(cfg, semana):
...
def test_el_backtest_nunca_supera_los_topes(cfg, instantanea, monkeypatch):
    ...
    ajustada = _con(
        ...
        "fundamental.activo",
        False,
        "cartera.max_posiciones",
        4,
        "cartera.max_por_mercado",
        2,
        "cartera.max_por_sector",
        2,
    )
