...
def test_con_historico_previo_se_decide_desde_el_primer_dia(cfg, instantanea, monkeypatch):
    ...
    dias, _ = _dias_de_revision(instantanea, cfg, inicio, dt.date(2023, 6, 30), monkeypatch)
    assert dias
    # La primera revision es la de la primera semana, no la de dentro de un ano.
    assert dias[0] - inicio < dt.timedelta(days=10)


def test_sin_historico_previo_se_sigue_esperando_el_calentamiento(cfg, instantanea, monkeypatch):
    rango_ini, _ = instantanea.rango_precios
    dias, _ = _dias_de_revision(instantanea, cfg, rango_ini, dt.date(2021, 12, 31), monkeypatch)
    assert dias
...
def test_los_anos_encadenados_dan_la_rentabilidad_total():
    ...
    r = dict(zip(tabla["ano"], tabla["rentabilidad"], strict=True))
...
def _cfg_minima():
    ...
    return SimpleNamespace(
        reglas=SimpleNamespace(
            metricas=SimpleNamespace(periodicidad_sharpe="semanal", tasa_libre_riesgo_anual=0.0)
        )
    )
