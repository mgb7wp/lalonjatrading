...
def _con(cfg, *pares):
    ...
    for ruta, valor in zip(pares[::2], pares[1::2], strict=True):
        partes = ruta.split(".")
        actual = datos
        for p in partes[:-1]:
            actual = actual[p]
...
def test_una_venta_sin_precio_se_aplaza_a_la_siguiente_sesion(cfg, instantanea):
    ...
    despues = ev[
        (ev["tipo"] == "venta") & (ev["ticker"] == salida["ticker"]) & (ev["fecha"] > dia_venta)
    ]
...
def test_sin_cambio_de_una_divisa_se_anota_el_motivo(cfg, instantanea):
    ...
    ev = backtest_mod.ejecutar(
        sin_brl, tecnico, dt.date(2021, 1, 1), dt.date(2022, 12, 31)
    ).eventos_df
...
def test_senales_detalle_ensena_los_rechazos_de_la_ultima_revision(cfg, monkeypatch, capsys):
    ...
    falsa = SimpleNamespace(preparar=lambda c: falsa, rango_precios=(dt.date(2020, 1, 1), d_venta))
...
def test_los_dias_sin_precio_salen_de_la_configuracion(cfg):
    ...
    inst = instantanea_de(
        cfg, serie_precios(ticker, fechas, [10.0 + i * 0.01 for i in range(len(fechas))])
    )
...
def test_el_margen_de_reserva_sale_de_la_configuracion(cfg):
    ...
    cand = Candidata(
        ...
        mercado="es",
        sector="industrial",
        fecha_decision=dt.date(2024, 6, 7),
        puntuacion_fundamental=50.0,
        percentil_momentum=50.0,
        puntuacion_final=50.0,
        senal=senal,
        n_cohorte=10,
        cohorte_usada="mercado",
    )
    ...
    justo = (
        nominal * (1 + cfg.reglas.cartera.margen_reserva_pct) + cfg.reglas.costes.comision_fija_eur
    )
