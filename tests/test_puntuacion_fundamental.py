...
def _ratios(ticker, roe=0.20, margen=0.15, ev_ebit=None):
    return Ratios(
        ticker=ticker, roe=roe, margen_operativo=margen, crecimiento_ventas_3a=0.05,
        flujo_caja_libre=1.0, deuda_neta_ebitda=1.0, ev_ebit=ev_ebit,
...
def _ultimo(**kw):
    base = {
        "ev": None, "acciones_en_circulacion": 1_000.0, "deuda_neta": 500.0,
...
def test_el_ev_usa_el_precio_sin_ajustar(cfg, instantanea, monkeypatch):
    ...
    con_dividendos = Instantanea(
        precios=precios, fundamentales=instantanea.fundamentales,
        fx=instantanea.fx, sectores=instantanea.sectores,
        fecha_descarga=instantanea.fecha_descarga, origen=instantanea.origen,
    ).preparar(cfg)
...
def _eventos(cohortes):
    return pd.DataFrame(
        [
            {"fecha": dt.date(2024, 1, 5), "tipo": "orden", "mercado": m, "ticker": f"T{i}",
             "motivo": "", "cohorte": c, "n_cohorte": 3}
