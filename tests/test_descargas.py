...
def _fila(ticker, fecha, o, h, lo, c, v=1_000.0):
    return {
        "fecha": fecha, "ticker": ticker, "apertura": o, "maximo": h, "minimo": lo,
        "cierre": c, "cierre_bruto": c, "volumen": v,
    }
...
def _fx(divisas, desde, hasta):
    ...
    return pd.DataFrame(
        [{"fecha": f, "divisa": d, "tasa": 1.1} for d in divisas for f in fechas]
    )
...
def test_un_volumen_todo_huecos_no_es_liquido(cfg):
    ...
    fechas = list(pd.bdate_range("2024-01-01", periods=80).date)
    inst = instantanea_de(
        cfg, serie_precios(ticker, fechas, [10.0] * 80, volumen=[np.nan] * 80)
    )
    medio = universo_mod.volumen_medio_base(ticker, "es", fechas[-1], inst.vista(fechas[-1]), cfg)
    assert medio == 0.0
...
def _fundamentales_desordenados(ticker):
    ...
    for ano, acciones in ((2023, 50.0), (2022, 50.0), (2021, 50.0), (2020, None)):
        filas.append(
            {
                "ticker": ticker, "fin_periodo": dt.date(ano, 12, 31), "periodo": "anual",
                "fecha_publicacion": dt.date(ano + 1, 4, 30),
                "origen_fecha_publicacion": "estimada_retraso", "origen_pit": "reconstruido",
                "roe": 0.2, "margen_operativo": 0.15, "ventas": 1000.0,
                "flujo_caja_libre": 90.0, "deuda_neta": 200.0, "ebitda": 200.0,
                "ebit": 150.0, "ev": None, "patrimonio_neto": 800.0,
                "acciones_en_circulacion": acciones, "divisa_reporte": "EUR",
